...
$query = mysqli_query($koneksi,"INSERT INTO pesanan (nama_penerima, user_id, nomor_telepon, kota_id, alamat, tanggal_pemesanan, status)
                                                      VALUES ('$nama_penerima', '$user_id', '$telepon', '$kota', '$alamat', '$waktu', '0')");
                                                      
if($query){
   //menyimpan id terakhir dari id pesanan ke pesanan_detail
   $last_pesanan_id=mysqli_insert_id($koneksi);
   $keranjang=$_SESSION["keranjang"];

   // baru dimasukan dari session keranjang ke database
